...
class Factor:
    def __init__(self, domain, values):
        ...
        ), f"invalid shape for values array {values.ndim}, {values.shape}, {domain.shape}"
        self.domain = domain
    ...
    def __iadd__(self, other):
        ...
        if len(self.domain) == 0:
            values = np.broadcast_to(self.values, other.domain.shape)
        if self.domain == other.domain:
            values = other.values
        else:
            values = other.expand(self.domain).values
        self.values += values
        return self
