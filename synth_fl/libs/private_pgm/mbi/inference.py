...
from scipy.sparse.linalg import LinearOperator, aslinearoperator, eigsh, lsmr
...
class FactoredInference:
    ...
    # Algorithm 2 - Accelerated Proximal Estimation
    # This estimates marginals mu and a graphical model that admits mu as its marginals
    ...
    def _marginal_loss(self, marginals, metric=None):
        ...
        for cl in marginals:
            ...
            for Q, y, noise, proj in self.groups[cl]:
                c = (
                    1.0 / noise if noise > 0 else 1
                )  # Noise correction as described in Appendix of PGM paper
                mu2 = mu.project(proj)
                x = mu2.datavector()
                diff = c * (Q @ x - y)
                if metric == "L1":
                    ...
                    loss += 0.5 * (diff @ diff)
                    # grad = c * (Q.T @ diff) # todo: performance hack for Q=I (no transpose)
                    grad = c * (Q @ diff)
