...
class Dataset:
    ...
    @staticmethod
    def load(path, domain, subset=None):
    ...
    def fast_project_flatten(self, cols):
        # data = self.df.loc[:, cols].values
        data = self.df[list(cols)].values
        domain_shape = [self.domain[attr] for attr in cols]
        bins = [range(n + 1) for n in domain_shape]
        ans = np.histogramdd(data, bins, weights=self.weights)[0]
        return ans.flatten()
