...
from synth_fl.libs.ektelo.algorithm.privBayes import privBayesSelect
from synth_fl.libs.ektelo.matrix import Identity
from synth_fl.libs.private_pgm.mbi import (Dataset, Factor, FactoredInference,
                                           mechanism)
from synth_fl.utils import generate_seed
from synth_fl.utils.privacy import (ExponentialMechanism, GaussianMechanism,
                                    zCDPAccountant)
...
def privbayes_measurements(data, eps=1.0, seed=0):
    ...
    ans = privBayesSelect.py_get_model(values, config, eps / 2, 1.0, seed, 0)
    ...
    for proj in projections:
        ...
        I = Identity(x.size)
        # TODO: sensitivity is 2 and does L2_sens / (eps/2)
...
def privbayes_measurements_zcdp(data, eps=1.0, seed=0):
    domain = data.domain
    config = ""
    for a in domain:
        values = [str(i) for i in range(domain[a])]
    ...
    accountant = zCDPAccountant(eps / 2, 1e-5)
    exp_mech = ExponentialMechanism(accountant=accountant)
    exp_sigma = exp_mech.get_cdp_sigma(
        num_rounds=1, rho=accountant.rho
    )  # Is scaled later by privBayes_model.cpp

    ans = privBayesSelect.py_get_model(values, config, eps / 2, 1.0, seed, exp_sigma)
    ans = ans.decode("utf-8")[:-1]
    ...
    sigma = gauss_mech.get_cdp_sigma(len(projections), rho=accountant.rho)

    measurements = []
    for proj in projections:
        x = data.project(proj).datavector()
        I = Identity(x.size)
        y = gauss_mech.apply(I.dot(x), sigma=sigma, sensitivity=2)
        measurements.append((I, y, 1.0, proj))
...
def privbayes_inference(domain, measurements, total):
    ...
    return Dataset(synthetic, domain), synthetic
