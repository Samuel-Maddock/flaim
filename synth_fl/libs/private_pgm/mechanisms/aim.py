...
from synth_fl.libs.private_pgm.mbi import Dataset, FactoredInference, GraphicalModel
...
class AIM(Mechanism):
    def __init__(
        ...
        self.model = None  # Hold final PGM model
        if enable_logging:
    ...
    def run(self, data, W):
        ...
        self.logger.debug(f"Size of workload (after downward closure): {len(workload)}")
        ...
        measurements = []
        self.logger.debug(f"Initial Sigma {sigma}")
        ...
        while not terminate:
            ...
            self.logger.debug(
                f"Original candidate size - size_limit={size_limit} - {len(candidates)}, new size = {len(small_candidates)}"
            )
            ...
            self.logger.debug(
                f"Selected {cl} Size {n}, Model size={hypothetical_model_size(model.domain, model.cliques)} Budget Used {rho_used/self.rho}"
            )
            if np.linalg.norm(w - z, 1) <= sigma * np.sqrt(2 / np.pi) * n:
                self.logger.debug(f"(!!!!!!!!!!!!!!!!!!!!!!) Reducing sigma {sigma/2}")
                sigma /= 2
                epsilon *= 2

        self.logger.debug("Generating Data...")
        engine.iters = 25  # TODO: Iters was originally 2500
        model = engine.estimate(measurements)
        self.model = model
