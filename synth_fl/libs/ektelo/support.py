...
import numpy as np
from ektelo import util, workload
...
def get_partition_vec(rank, n, cluster, closeRange=False):
    ...
    for i in range(len(cluster)):
        if closeRange:
            assert (
                cluster[-1][1] == n - 1
            ), " Last bin of partition must end with length of original data"
...
def get_subdomain_grid(mapping, domain_shape):
    ...
    for g in corners:
        temp = corners[g]
        sub_domains[g] = (
            temp["ll"][0] - temp["ul"][0] + 1,
            temp["ur"][1] - temp["ul"][1] + 1,
        )
...
def mapping_statistics(mapping):
    return np.unique(
        mapping, return_index=True, return_inverse=True, return_counts=True
    )
...
def reduction_matrix(mapping, canonical_order=False):
    ...
    assert (
        mapping.ndim == 1
    ), "Can only handle 1-dimesional mappings for now, domain should be flattened"
    ...
    return EkteloMatrix(
        sparse.csr_matrix((data, (rows, cols)), shape=(m, n), dtype=int)
    )
