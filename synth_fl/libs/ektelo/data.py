    ...
    def materialize(self, X, prng, node):
        ...
        paths = [
            ...
            for path in nx.all_simple_paths(
                self._dgraph.graph, self._dgraph.root.id, node.id
            )
        ]
...
class RelationHelper(object):
    ...
    resource_config_file_map = {
        "CPS": "cps.yml",
        "STROKE_1D": "stroke.yml",
        "STROKE_2D": "stroke.yml",
    }
    resource_config_name_map = {
        "CPS": "cps_config",
...
class Histogram(Marshallable):
    ...
    def generate(self):
        self.hist, self.edges = np.histogramdd(
            self.df.values, self.bins, self.domains, self.normed, self.weights
        )
    ...
    def statistics(self):
        ...
        hist_data = {}
        hist_data["nz_perc"] = util.old_div(
            np.count_nonzero(self.hist), float(self.hist.size)
        )
...
class Relation(Marshallable):
    ...
    def value_map(self):
        return [
            (
                self.config[column]["value_map"]
                if "value_map" in self.config[column]
                else None
            )
    ...
    def project(self, fields):
        ...
        assert all_fields.issuperset(
            set(fields)
        ), "supplied fields are not subset of config fields"

        self._df = self._df[fields]
        config_pairs = [
            (key, value) for key, value in self.config.items() if key in fields
        ]
