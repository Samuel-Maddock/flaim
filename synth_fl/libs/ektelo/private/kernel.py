...
from ektelo.data import DataManager, Graph, Node, Relation, RelationHelper
from ektelo.private import transformation


class PrivateManager(object):
    def __init__(
        self, source_uri, data_config, file_delimiter=",", random_seed=0, budget=1.0
    ):
    ...
    def _map(self, mapping, parent_node):
        ...
        for idx in set(mapping.vector):
            ...
            groups.append(
                self._tgraph.insert_transformation(
                    transformation.Group(idxs), parent_node
                )
            )
