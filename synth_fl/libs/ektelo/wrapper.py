...
from .client import inference, mapper
from .client import measurement as cmeasurement
from .client import selection
from .private import measurement, pselection
...
def multiplicative_weights(Ms, ys, x_est, update_rounds=50, scale_factors=None):
    return inference.MultiplicativeWeights(update_rounds).infer(
        Ms, ys, x_est, scale_factors
    )
...
def non_negative_least_squares(Ms, ys, l1_reg=0, l2_reg=0, scale_factors=None):
    return inference.NonNegativeLeastSquares(l1_reg, l2_reg).infer(
        Ms, ys, scale_factors
    )
...
def marginal_partition(domain, proj_dim):
    return mapper.MarginalPartition(domain_shape=domain, proj_dim=proj_dim).mapping()
