...
import ektelo
import numpy as np
from ektelo import matrix, support, util, workload
...
def GenerateCells(n, m, num1, num2, grid):
    ...
    """
    assert (
        math.ceil(util.old_div(n, float(grid))) == num1
        and math.ceil(util.old_div(m, float(grid))) == num2
    ), "Unable to generate cells for Ugrid: check grid number and grid size"
    lower, upper = [], []
    for i in range(num1):
        for j in range(num2):
...
class HierarchicalRanges(SelectionOperator):
    ...
    def expand_offsets(cur_rect_l, cur_rect_u, offsets):
        ...
        assert all(
            [len(d) == len(offsets[0]) for d in offsets]
        ), "Shape of offsets along each dimension should match."
        if len(offsets[0]) < 1:
            return [], []
        # expand offsets across different levels.
        expanded_offsets = [
            HierarchicalRanges.quick_product(*d).sum(axis=0) for d in offsets
        ]
        lower = np.vstack(
            [l + offset for l, offset in zip(cur_rect_l, expanded_offsets)]
        ).T
        upper = np.vstack(
            [u + offset for u, offset in zip(cur_rect_u, expanded_offsets)]
        ).T
        return lower, upper
    ...
    def grid_split_range(cur_range_l, cur_range_u, **kwargs):
        ...
        assert len(branchings) == len(
            dim_lens
        ), "The numbers of dimension and branching factors need to match"
        ...
        def get_boarder(dim_len, branching):
            if branching > dim_len:
                ...
                boarder = [
                    (i * cell_size_h, (i + 1) * cell_size_h - 1)
    ...
    @staticmethod
    def ranges_gen(
        start_range_l, start_range_u, include_root, include_leaf, split_func, **kwargs
    ):
        """
        ...
        **kwargs: argument list, will be passed into split_func
        ...
        pending_offsets = (
            [[[np.array([0], dtype="int")]] * dimension]
            if include_root
        ...
        while len(pending_l) != 0:
            ...
            lower, higher = HierarchicalRanges.expand_offsets(
                cur_range_l, cur_range_u, cur_offset
            )
            ...
            same_shape_groups = HierarchicalRanges.same_shape(
                sub_ranges_l, sub_ranges_u
            )
            for group_idx in same_shape_groups:
                ...
                canonical_range_l, canonical_range_u = (
                    sub_ranges_l_group[0],
                    sub_ranges_u_group[0],
                )
                canonical_shape = [
                    u - l + 1 for l, u in zip(canonical_range_l, canonical_range_u)
                ]
                ...
                if np.prod(canonical_shape) > 1:
                    ...
                    for d in range(len(cur_offset)):
                        offset_level = (
                            np.array(sub_ranges_l_group)[:, d] - canonical_range_l[d]
                        )
...
class QuadTree(HierarchicalRanges):
    ...
    def __init__(self, domain_shape, fast_leaves=True):
        assert len(domain_shape) == 2, "QuadTree selection is defined on 2D"
        domain_shape = domain_shape
...
class Identity(SelectionOperator):
    def __init__(self, domain_shape):
        ...
        assert (
            isinstance(domain_shape, tuple) and len(domain_shape) == 1
        ), "Identity selection only workss for 1D"
        self.domain_shape = domain_shape
...
class Total(SelectionOperator):
    def __init__(self, domain_shape):
        ...
        assert (
            isinstance(domain_shape, tuple) and len(domain_shape) == 1
        ), "Total selection only works for 1D"
        self.domain_shape = domain_shape
...
class GreedyH(SelectionOperator):
    def __init__(self, domain_shape, W, branch=2, granu=100):
        ...
        assert (
            isinstance(domain_shape, tuple) and len(domain_shape) == 1
        ), "greedyH selection only supports 1D  domain shapes"
        self.domain_shape = domain_shape
        self.W = W
        self._branch = branch
        self._granu = granu
    ...
    def select(self):
        ...
        err, inv, weights, queries = self._GreedyHierByLv(QtQ, n, 0, withRoot=False)
    ...
    def _GreedyHierByLv(self, fullQtQ, n, offset, depth=0, withRoot=False):
        ...
        if n == 1:
            return (
                np.linalg.norm(fullQtQ[:, offset], 2) ** 2,
                np.array([[1.0]]),
                np.array([1.0]),
                [[offset, offset]],
            )
        ...
        m1 = sum(
            map(
                lambda rng, v: np.linalg.norm(np.dot(QtQ[:, rng[0] : rng[1]], v), 2)
                ** 2,
                bound,
                invAuList,
            )
        ...
        toterr = (
            1.0 / err1 * (sumerr - ((m - m1) * decay + m1) * err2 / (err1 + err2 * k))
        )
...
class UniformGrid(SelectionOperator):
    def __init__(self, domain_shape, data_sum, esp_par, ag_flag=False, c=10, gz=0):
        assert (
            isinstance(domain_shape, tuple) and len(domain_shape) == 2
        ), "UniformGrid selection only workss for 2D"
...
class AdaptiveGrid(HierarchicalRanges):
    def __init__(self, domain_shape, x_hat, eps_par, c2=5):
        assert (
            isinstance(domain_shape, tuple) and len(domain_shape) == 2
        ), "AdptiveGrid selection only works for 2D"
    ...
    def select(self):
        ...
        if shape == (1, 1):
            ...
            return workload.RangeQueries(
                (1, 1), lower=np.array([[0, 0]]), higher=np.array([[0, 0]])
            )
        ...
        return workload.RangeQueries(
            self.domain_shape, np.array(lower), np.array(higher)
        )
...
class HDMarginal(SelectionOperator):
    ...
    def select(self):
        ...
        for ind, shape in enumerate(domain_shape):
            queries = (
                [np.ones(n) for n in domain_shape[:ind]]
                + [sparse.identity(shape)]
                + [np.ones(n) for n in domain_shape[ind + 1 :]]
            )
