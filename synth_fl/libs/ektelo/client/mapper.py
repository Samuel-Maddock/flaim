...
import numpy as np
from ektelo import support, util
...
class AHPCluster(MapperOperator):
    ...
    def mapping(self):
        ...
        partition_vec = support.get_partition_vec(
            rank, len(sorted_xest), cluster, closeRange=False
        )
...
class UGridPartition(MapperOperator):
    ...
    def GenerateCells(n, m, num1, num2, grid):
        ...
        assert (
            math.ceil(n / float(grid)) == num1 and math.ceil(m / float(grid)) == num2
...
class MarginalPartition(MapperOperator):
    ...
    def mapping(self):
        ...
        margin_vec[self.proj_dim] = np.arange(
            self.domain_shape[self.proj_dim]
