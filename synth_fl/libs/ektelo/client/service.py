...
import numpy as np
from ektelo.client import inference, mapper, selection
from ektelo.data import DataManager, RelationHelper
from ektelo.private import kernel, measurement, meta, pmapper, pselection
...
class ProtectedDataSource(object):
    ...
    def instance(source_uri, random_seed=0):
        ...
        private_manager = kernel.PrivateManager(
            relation_helper.filename, relation_helper.config, random_seed=random_seed
        )
        kernel_service = pservice.KernelService(private_manager)

        return ProtectedDataSource(kernel_service, DataManager(transformation.Null()))

    def transform(self, class_name, init_params={}):
        private_node_id = self.kernel_service.transform(
            self.private_node_id, class_name, init_params
        )

        data_node = self.data_manager.transform(
            getattr(transformation, class_name)(*init_params), self.data_node
        )

        return self._clone(private_node_id, data_node)

    def measure(self, class_name, eps, init_params={}):
        return self.kernel_service.measure(
            self.private_node_id, class_name, eps, init_params
        )

    def select(self, class_name, eps, init_params={}):
        return self.kernel_service.select(
            self.private_node_id, class_name, eps, init_params
        )

    def mapping(self, class_name, eps, init_params={}):
        return self.kernel_service.mapping(
            self.private_node_id, class_name, eps, init_params
        )

    def dawa(self, ratio, approx, eps):
        return self.mapping(
            pmapper.Dawa.__name__, eps, {"eps": eps, "ratio": ratio, "approx": approx}
        )
    ...
    def vectorize(self, reduced_domain):
        return self.transform(
            transformation.Vectorize.__name__,
            {"name": "", "reduced_domain": reduced_domain},
        )
    ...
    def reduce_by_partition(self, mapping):
        return self.transform(
            transformation.ReduceByPartition.__name__, {"mapping": mapping}
        )
    ...
    def priv_bayes_select(self, theta, domain, eps):
        return self.select(
            pselection.PrivBayesSelect.__name__,
            eps,
    ...
    def worst_approx(self, W, measuredQueries, x_hat, eps, mechanism="EXPONENTIAL"):
        return self.select(
            pselection.WorstApprox.__name__,
            eps,
            {
                "W": W,
                "measuredQueries": measuredQueries,
                "x_est": x_hat,
                "eps": eps,
                "mechanism": mechanism,
            },
    ...
    def _clone(self, private_node, data_node):
        return ProtectedDataSource(
            self.kernel_service, self.data_manager, private_node, data_node
        )
