...
from ektelo.private import measurement, meta, pmapper, pselection, transformation
...
class PrivBayesLS(Base):
    ...
    def Run(self, W, relation, eps, seed):
        prng = np.random.RandomState(seed)
        M = pselection.PrivBayesSelect(self.theta, self.domain, eps).select(
            relation, prng
        )
...
class Mwem(Base):
    ...
    def __init__(
        self, ratio, rounds, data_scale, domain_shape, use_history, update_rounds=50
    ):
        self.init_params = util.init_params_from_locals(locals())
        self.ratio = ratio
        self.rounds = rounds
    ...
    def Run(self, W, x, eps, seed):
        ...
        for i in range(1, self.rounds + 1):
            ...
            worst_approx = pselection.WorstApprox(
                W, measuredQueries, x_hat, eps_round * self.ratio, "EXPONENTIAL"
            )
...
class AGrid(Base):
    ...
    def Run(self, W, x, eps, seed):
        ...
        M = selection.UniformGrid(
            shape_2d, self.data_scale, eps, ag_flag=True, c=self.c
        ).select()
        ...
        uniform_mapping = mapper.UGridPartition(
            shape_2d, self.data_scale, eps, ag_flag=True, c=self.c
        ).mapping()
        ...
        for i in sorted(set(uniform_mapping)):
            ...
            M_i = selection.AdaptiveGrid(
                sub_domain_shape, x_hat_i, (1 - self.alpha) * eps, c2=self.c2
            ).select()
...
class DawaStriped(Base):
    ...
    def Run(self, W, x, eps, seed):
        ...
        for i in group_idx:
            ...
            M_bar = selection.GreedyH(x_bar.shape, W_bar).select()
            y_i = measurement.Laplace(M_bar, eps * (1 - self.ratio)).measure(
                x_bar, prng
            )

            noise_scale_factor = laplace_scale_factor(M_bar, eps * (1 - self.ratio))
...
class MwemVariantB(Base):
    ...
    def Run(self, W, x, eps, seed):
        ...
        for i in range(1, self.rounds + 1):
            ...
            worst_approx = pselection.WorstApprox(
                W, measuredQueries, x_hat, eps_round * self.ratio
            )
    ...
    def Run(self, W, x, eps, seed):
        ...
        for i in range(domain_dimension):
            ...
            marginal_mapping = mapper.MarginalPartition(
                domain_shape=self.domain_shape, proj_dim=i
            ).mapping()
            ...
            if self.domain_shape[i] < 50:
                ...
                M_bar = selection.GreedyH(x_bar.shape, W_bar).select()
                y_i = measurement.Laplace(M_bar, eps_share * (1 - self.ratio)).measure(
                    x_bar, prng
                )
