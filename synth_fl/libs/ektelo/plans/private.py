        ...
        for i in range(1, self.rounds + 1):
            ...
            if self.use_history:
                x_hat = multiplicative_weights(
                    M_history, y_history, x_hat, update_rounds=self.update_rounds
                )
            else:
                x_hat = multiplicative_weights(
...
class UGrid(Base):
    ...
    def Run(self, W, x, eps):
        assert (
            len(self.domain_shape) == 2
        ), "Uniform Grid only works for 2D domain_shape"
...
class AGrid(Base):
    ...
    def Run(self, W, x, eps):
        assert (
            len(self.domain_shape) == 2
        ), "Adaptive Grid only works for 2D domain_shape"

        shape_2d = self.domain_shape
        Ms = []
        ys = []

        M = ugrid_select(shape_2d, self.data_scale, eps, ag_flag=True, c=self.c)
...
class MwemVariantB(Base):
    ...
    def Run(self, W, x, eps):
        ...
        for i in range(1, self.rounds + 1):
            ...
            # SW + SH2
            M = x.worst_approx(W, measuredQueries, x_hat, eps_round * self.ratio)
    ...
    def Run(self, W, x, eps):
        ...
        for i in range(1, self.rounds + 1):
            ...
            x_hat = non_negative_least_squares(
                M_history, y_history, 1e-6, 1e-6, noise_scales
            )
