...
from ektelo.mixins import CartesianInstantiable, Marshallable
...
class Dataset(Marshallable):
    def __init__(self, hist, reduce_to_domain_shape=None, dist=None):
        ...
        """
        if isinstance(
            reduce_to_domain_shape, int
        ):  # allow for integers in 1D, instead of shape tuples
        ...
        self._hist = hist
        self._reduce_to_domain_shape = (
            reduce_to_domain_shape if hist.shape != reduce_to_domain_shape else None
        )
        self._dist = dist
        self._payload = None
    ...
    def reduce_data(self, p_grid, data):
        ...
        assert (
            data.shape == p_grid.shape
        ), "Shape of x and shape of partition vector must match."
        # get out_shape
        if p_grid.ndim == 1:
            ...
            out_shape = tuple(out)
        # reduce
        unique, indices, inverse, counts = numpy.unique(
            p_grid, return_index=True, return_inverse=True, return_counts=True
        )
        res = numpy.zeros_like(unique, dtype=float)
        for index, c in numpy.ndenumerate(
            data.ravel()
        ):  # needs to be flattened for parallel indexing with output of unique
            res[inverse[index]] += c
    ...
    def compile(self):
        ...
        if self._reduce_to_domain_shape:  # reduce cells to get desired shape
            q = [
                divmod(dom, grid)
                for (dom, grid) in zip(
                    self._payload.shape, self._reduce_to_domain_shape
                )
            ]
            for red, rem in q:
                assert rem == 0, (
                    "Domain must be reducible to target domain by uniform grid: %i, %i"
                    % (red, rem)
                )
        ...
        self._compiled = True
        self._payload = self._payload.astype(
            "int"
        )  # partition engines need payload to be of type int
...
class DatasetFromRelation(Dataset, CartesianInstantiable):
    def __init__(
        self, relation, nickname, normed=False, weights=None, reduce_to_dom_shape=None
    ):
        self.init_params = util.init_params_from_locals(locals())
        self.fname = nickname

        histogram = Histogram(
            relation.df, relation.bins, relation.domains, normed, weights
        ).generate()
        ...
        super(DatasetFromRelation, self).__init__(
            histogram.hist.copy(order="C"), reduce_to_dom_shape
        )

    @staticmethod
    def instantiate(params):
        relation = data.Relation(params["config"]).load_csv(
            params["csv_filename"], params["csv_delimiter"]
        )

        return DatasetFromRelation(
            relation,
            params["nickname"],
            params["normed"],
            params["weights"],
...
class DatasetUniformityVaryingSynthetic(Dataset, CartesianInstantiable):
    def __init__(self, uniformity, dom_shape, scale, seed=None):
        ...
        super(DatasetUniformityVaryingSynthetic, self).__init__(
            hist.reshape(dom_shape), reduce_to_domain_shape=None, dist=None
        )

    @staticmethod
    def instantiate(params):
        return DatasetUniformityVaryingSynthetic(
            params["uniformity"], params["dom_shape"], params["scale"], params["seed"]
        )
...
def load(filename):
    ...
    fullpath = os.path.join(
        path[0], "datafiles", filename
    )  # fixed for now, so we don't have to include as a parameter in automated key extraction stuff...
...
def subSample(dist, sampleSize, prng):
    ...
    samples = prng.choice(
        a=dist.size, replace=True, size=int(sampleSize), p=dist.flatten()
    )
    hist = numpy.histogram(samples, bins=dist.size, range=(0, dist.size))[0].astype(
        "int32"
    )  # return only counts (not bins)
...
def canonicalTransform(vector):
    ...
    unique, indices, inverse = numpy.unique(
        vector, return_index=True, return_inverse=True
    )
    ...
    tups = list(
        zip(uniqueInverse, newIndex)
    )  # replace uniqueInverse with unique if we want to use the exact numbers in partition vector
...
def partition_grid(domain_shape, grid_shape):
    ...
    assert (
        sum(divmod(d, b)[1] for (d, b) in zip(domain_shape, grid_shape)) == 0
    ), "Domain size along each dimension should be a multiple of size of block"
...
if __name__ == "__main__":
    ...
    for u in [util.old_div(i, 10.0) for i in range(0, 11)]:
        print(u)
        d = DatasetUniformityVaryingSynthetic(
            uniformity=u, dom_shape=(10,), scale=scale, seed=999
        )
        size = d.payload.size
