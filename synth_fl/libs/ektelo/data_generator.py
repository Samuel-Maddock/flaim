...
def make_array(
    sample_size, universe_size, sparsity_fraction=0.8, power=1.2, prng=numpy.random
):
...
def make_hist(
    sample_size=312467538, shape=None, s_frac=0.8, power=1.2, prng=numpy.random
):
    ...
    array_data = make_array(
        sample_size, numpy.prod(shape), sparsity_fraction=s_frac, power=power, prng=prng
    )
...
def make_hist_using_factfinder(prng=numpy.random):
    ...
    hist_hispanic = [
        50477594,
        258267944,
    ]  # from https://factfinder.census.gov/faces/tableservices/jsf/pages/productview.xhtml?pid=DEC_10_SF1_P4&prodType=table
    ...
    hist_sex = [
        151781326,
        156964212,
    ]  # from https://factfinder.census.gov/faces/tableservices/jsf/pages/productview.xhtml?pid=DEC_10_SF1_P12&prodType=table
    ...
    while not myiter.finished:
        ...
        p[ind] = reduce(
            lambda x, y: x * y[1][y[0]] / float(sample_size),
            list(zip(ind, fields)),
            1.0,
        )
        myiter.iternext()
