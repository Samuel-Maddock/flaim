...
class ahp_engine(estimate_engine.estimate_engine):
    ...
    def Run(self, Q, x, epsilon, seed):
        ...
        assert len(x.shape) == 1, (
            "%s is defined for 1D data only" % self.__class__.__name__
        )
...
Invocation:         ektelo.algorithm.ahp.ahpND_engine()
...
class ahpND_engine(estimate_engine.estimate_engine):
    ...
    def Run(self, Q, x, epsilon, seed):
        ...
        shape = x.shape
        hatx1d = engine.Run(
            Q, np.reshape(x, x.size), epsilon, seed
        )  # call AHP on 1d array
...
Reference:          -
Invocation:         ektelo.algorithm.ahp.ahpND_adaptive_engine()
Implementation:     DPComp team
"""
