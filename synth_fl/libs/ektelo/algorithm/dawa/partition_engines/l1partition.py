...
from synth_fl.libs.ektelo.algorithm.dawa.cutils import cutil
from synth_fl.libs.ektelo.algorithm.dawa.partition_engines import partition_engine
        ...
        for lb in hist[1:]:
            hatx[lb:rb] = util.old_div(
                max(
                    0,
                    sum(x[lb:rb])
                    + prng.laplace(0, util.old_div(1.0, (epsilon * (1 - ratio))), 1),
                ),
                float(rb - lb),
            )
            rb = lb
