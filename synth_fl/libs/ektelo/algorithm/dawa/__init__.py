...
from synth_fl.libs.ektelo.algorithm import estimate_engine
...
Invocation:         ektelo.algorithm.dawa.dawa_engine()
...
Additional aliases: -
Reference:          [C. Li, M. Hay, and G. Miklau. A data- and workload-aware algorithm for range queries under differential privacy. PVLDB, 2014.](http://dl.acm.org/citation.cfm?id=2732271) 
Invocation:         ektelo.algorithm.dawa.dawa2D_engine()
Implementation:     DPComp team
"""
...
class dawa2D_engine(routine_engine.transform_engine_qtqmatrix_linear, Marshallable):
    ...
    def Run(self, Q, x, epsilon, seed):
        ...
        assert len(x.shape) == 2, (
            "%s is defined for 2D data only" % self.__class__.__name__
        )
        n1, n2 = x.shape
        Q = list(Q.query_list)
        ...
        xcoords, ycoords = hilbert(
            d
        )  # coordinates of points along Hilbert curve on d x d square
        x1d = x[
            xcoords, ycoords
        ]  # index into x at these points to get 1d representation

        # STEP 2: similarly transform queries
        m = len(Q)
        Qmat = numpy.zeros(
            [m, len(x1d)]
        )  # workload matrix: m by n, for m queries over 1d domain of size n
        for i in range(m):
            query = Q[i]
            query2d = query.asArray([d, d])  # query as d x d array
            query1d = query2d[
                xcoords, ycoords
            ]  # array transformed to 1d via hilbert transform
        ...
        hatx2d = numpy.zeros([d, d])

        hatx2d[
            xcoords, ycoords
        ] = hatx1d  # assign the values of hatx1d into hatx2d at indexes (xcoords, ycoords)
        hatx2d = hatx2d[
            :n1, :n2
        ]  # remove excess: extract out the n1 x n2 portion of hatx2d
...
Reference:          [C. Li, M. Hay, and G. Miklau. A data- and workload-aware algorithm for range queries under differential privacy. PVLDB, 2014.](http://dl.acm.org/citation.cfm?id=2732271) 
Invocation:         ektelo.algorithm.dawa.greedyH_only_engine()
Implementation:     DPComp team
"""
...
def hilbert(N):
    ...
    """
    assert (
        2 ** int(math.ceil(math.log(N, 2))) == N
    ), "N={0} is not a power of 2!".format(N)
    if N == 2:
