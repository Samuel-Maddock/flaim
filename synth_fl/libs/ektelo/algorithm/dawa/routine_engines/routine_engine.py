...
from synth_fl.libs.ektelo import util, workload
...
class routine_engine(object):
    ...
    def Compute_partition(self, x, epsilon, pSeed):
        ...
        if self._ratio == 0:
            ...
            self._partition = self._partition_engine.Run(x, epsilon, self._ratio, pSeed)
...
class transform_engine_qtqmatrix(routine_engine):
    ...
    def __init__(self, p_engine=None, e_engine=None, ratio=0.5, max_block_size=None):
        """Setting up engines for each step.
...
class transform_engine_qtqmatrix_linear(transform_engine_qtqmatrix):
    ...
    def __init__(self, p_engine=None, e_engine=None, ratio=0.5):
        super(transform_engine_qtqmatrix_linear, self).__init__(
            p_engine, e_engine, ratio, None
        )
    ...
    def _workload_reform(self, Q0, partition, n):
        ...
        if (
            len(Q0.shape) == 3
        ):  # total hack: if the workload is range queries, then first tfm them to matrix
            # run "old" code (transform_engine_qtqmatrix) and save result to compare against
            QtQold = super(transform_engine_qtqmatrix_linear, self)._workload_reform(
                Q0, partition, n
            )
            Q0 = as_matrix(Q0, n)
        else:
        ...
        assert (
            partition[-1][-1] + 1
        ) == n  # these should be equal, not sure why Chao computes n from partition?
        n2 = len(partition)
        QtQ = numpy.zeros([n2, n2])
        ...
        for c in range(n2):
            lb, rb = partition[
                c
            ]  # column c of Qmat represents query weight on c^th partition
            Qmat[:, c] = Q0mat[:, lb : (rb + 1)].mean(
                axis=1
            )  # query weight is average of weights on bins in that partition

        QtQ += numpy.dot(Qmat.T, Qmat)

        if (
            QtQold is not None
        ):  # debugging purposes: compare against transform_engine_qtqmatrix
