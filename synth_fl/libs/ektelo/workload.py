...
from synth_fl.libs.ektelo import matrix
from synth_fl.libs.ektelo.matrix import (
    EkteloMatrix,
    Identity,
    Kronecker,
    Ones,
...
class RangeQueries(matrix.Product):
    ...
    def unproject(self, offset, domain):
        return RangeQueries(
            domain, self._lower + np.array(offset), self._higher + np.array(offset)
        )
...
class Marginals(VStack):
    def __init__(self, domain, weights):
        ...
        for key, wgt in enumerate(weights):
            if wgt > 0:
                subs.append(wgt * Marginal(domain, key))
